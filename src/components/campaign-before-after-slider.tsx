...
export function CampaignBeforeAfterSlider({
  beforeImage,
  afterImage,
  beforeLabel,
  afterLabel,
  ...
  return (
    <div className="rounded-2xl border border-ocean-900/10 bg-white p-6 shadow-soft">
      <div className="relative h-[360px] overflow-hidden rounded-2xl bg-ocean-900">
        <Image src={afterImage} alt={afterLabel} fill unoptimized className="object-cover" sizes="(min-width: 1024px) 760px, 100vw" />
        <div className="absolute inset-y-0 left-0 overflow-hidden" style={{ width: `${position}%` }}>
          <div className="relative h-full" style={{ width: `${10000 / position}%` }}>
            <Image src={beforeImage} alt={beforeLabel} fill unoptimized className="object-cover grayscale" sizes="(min-width: 1024px) 760px, 100vw" />
          </div>
        </div>
      ...
      <p className="mt-3 text-sm leading-6 text-ocean-900/62">{caption}</p>
    </div>
  );
}
