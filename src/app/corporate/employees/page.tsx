...
export default async function CorporateEmployeesPage({ searchParams }: CorporateEmployeesPageProps) {
  ...
  return (
    <main className="mx-auto max-w-6xl px-4 py-8 sm:px-6 lg:px-8">
      ...
      {successMessage ? (
        <p className="mt-6 rounded-lg border border-kelp-500/20 bg-kelp-100 px-4 py-3 text-sm font-bold text-kelp-700">{successMessage}</p>
      ) : null}
      {params?.error ? (
        <p className="mt-6 rounded-lg border border-coral-500/20 bg-coral-100 px-4 py-3 text-sm font-bold text-coral-700">Employee event, invite, or attendance could not be saved with the current input or permission.</p>
      ) : null}
      ...
      <section className="mt-6 grid gap-6 xl:grid-cols-[1fr_0.95fr_0.95fr]">
        <article className="rounded-lg border border-ocean-900/10 bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid gap-3">
            {data.employeePrograms.upcomingEvents.length > 0 ? data.employeePrograms.upcomingEvents.map((event) => {
              ...
              return (
                <div key={event.id} className="rounded-lg border border-ocean-900/10 bg-sand-50 p-4">
                  ...
                  {event.registrations.length > 0 ? (
                    <div className="mt-4 grid gap-2">
                      {event.registrations.slice(0, 5).map((registration) => (
                        <div key={registration.id} className="rounded-lg border border-ocean-900/10 bg-white px-3 py-3 text-sm">
                          ...
                          {canManageEmployees && registration.status !== "cancelled" ? (
                            <div className="mt-3 flex flex-wrap gap-2">
                              <form action={checkInCorporateEmployeeEventAction} className="flex flex-wrap items-center gap-2">
                                ...
                                <input name="attendanceHours" type="number" min="0" step="0.25" defaultValue={defaultHours} className="h-10 w-24 rounded-lg border border-ocean-900/12 px-3 text-sm font-semibold text-ocean-900 outline-none" />
              ...
              <p className="rounded-lg border border-dashed border-ocean-900/16 bg-sand-50 px-4 py-5 text-sm font-semibold leading-6 text-ocean-900/62">
        ...
        <article className="rounded-lg border border-ocean-900/10 bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid gap-2">
            {data.employeePrograms.challenge.leaderboard.map((department, index) => (
              <div key={department.department} className="flex items-center justify-between gap-3 rounded-lg bg-sand-50 px-4 py-3 text-sm">
        ...
        <article className="rounded-lg border border-ocean-900/10 bg-white p-5 shadow-soft">
          ...
          <div className="mt-5 grid gap-3">
            <p className="rounded-lg bg-sand-50 px-4 py-3 text-sm font-bold text-ocean-900">Pool {formatCurrency(data.employeePrograms.donationMatching.pool)}</p>
            <p className="rounded-lg bg-sand-50 px-4 py-3 text-sm font-bold text-ocean-900">Matched {formatCurrency(data.employeePrograms.donationMatching.matched)}</p>
            <p className="rounded-lg bg-sand-50 px-4 py-3 text-sm font-bold text-ocean-900">{data.employeePrograms.donationMatching.pending.toLocaleString("id-ID")} claims pending eligibility review</p>
          </div>
        </article>
      </section>
